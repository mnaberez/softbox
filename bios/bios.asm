...
;Revision 02-May-2014
;
;This is a modified version of the 09-June-1983 BIOS
;that fixes the bug in CBM 8250 support.
;
;SoftBox Memory Map:
...
;
;A = number (e.g. A=0Ah sends " 010", A=FFh sends " 255")
;
    push af             ;Save A
    ld a,' '            ;Send leading space
    call wrieee
    pop af              ;Recall A
    ld h,0              ;HL = A
    ld l,a
    ld bc,-100          ;Send hundreds place
    call num1
    ld c,-10            ;Send tens place
    call num1
    ld c,-1             ;Send ones place
num1:
    ld a,'0'-1          ;Seed A with 2Fh = ASCII "0" - 1
num2:
    inc a               ;Increment to next ASCII character
    add hl,bc
    jr c,num2
    sbc hl,bc
    call wrieee         ;Send ASCII character
    ret
